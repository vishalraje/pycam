...
of both libraries, assessing their performance as their usability.
We also describe how Python implementation of OpenCV performs in comparison
of the C++ implementation. Our lessons learnt as a list of recommendations
...
Python \cite{van1994python} has been growing of interest by the academic
...
syntax of python, the absence of strongly typed data, or hardware
resources management (e.g. memory management) has retained their attention
...
has been driven for the last decade by C/C++ implementation or the
usage of MATLAB \cite{}. As MATLAB offers a really efficient high
level platform for prototyping and testing algorithms, its performances
...
The outcomes of this evaluation aim to help on any decisional process
...
with the native C implementation. We also focused our attention on
the performances of two widely used CV libraries in Python: \textit{OpenCV}\cite{bradski2000opencv}
and \textit{NumPy/SciPy}\cite{oliphant2007python}. For this matter,
we analyzed their performances through a list of common tasks and
processes regularly employed in computer vision (e.g. video capture,
filtering algorithms, feature detection, etc).
...
(optionally to embedded platforms). The library is using a base layer
...
as OpenCV DotNet ( OpenCVDotNet, .NET Framework Wrapper for Intel's
OpenCV Package), the cross platform C\# EmguCV (http://www.emgu.com/wiki/index.php/Main\_Page),
GPUCV\cite{farrugia2006gpucv} and multiple for Python (automatically
Swig\cite{beazley1996swig} wrapped, OpenCV Python).


\subsection*{NumPy/SciPy}

\emph{NumPy} gives strongly typed N-dimensional array support to Python\cite{oliphant2006guide}.
It's a well recognized library, much of the lower level algorithms
are implemented in C and Fortran, offering very fast raw data crunching,
iterating, ande an easier approach for multidimensional array manipulation
...
SciPy offers many different filters, routines for numerical integration
and optimization. This leads to much more elegant code, usually achieved
without any significant performance loss over C. Two major tools that
are usually distributed with SciPy that are very usefull for computer
vision work are matplotlib and ipython. Matplotlib\cite{hunter2007matplotlib}
is a matrix plotting library, and IPython\cite{perez2007ipython}
...
1.1.1, rev 1978. The code has been compiled with the GNU toolchain
version (4.3.3), and in Release mode with O3 compiler optimisations,
MMX, fast math, SSE3. All additional packages, EXCEPT 1393, are turned
on, eg png, jpg, gtk, gstreamer, unicap, V4L turned on. 
...
We cover different standard algorithms traditionally used in a CV
application and also some standard operations used in CV such as video
grabbing or image recording. For each of the tests we describe the
process, the difference in term of the syntax, the performance and
the usability.

Notice that for some tests we only compare the Python OpenCV implementation
against the SciPy libraries, as disscussed later, the performance
of the wrapped code alone, made more testing redundant.
...
of CV applications, we tested just getting and showing a frame as
the most basic, but nessasary test.
...
Figure \ref{fig:Streaming-comparison} shows results for taking an
image from a webcam and displaying the image with OpenCV. The average
frames per second were taken with 3 iterations, each averaging the
framerate over a 2 minute period. Error bars are showing standard
deviation of sampled data. As Figure \ref{fig:Streaming-comparison}
...
higher frame rate output than Python. 
...
data from OpenCV. This means that as developers, to debug a misbehaving
program, we can just jump right into the middle of a deeply nested
loop, introspect all variables, plot any signal or image, call any
function, accuratly time any function. Two lines of Python code can
be put anywhere in a computer vision program and it will pause execution
...
If there is more than a threshold of differance, something changed.
...
While creating this program I found IPython particuly helpfull, it
allows for interactive, inplace trialing, timing and plotting as shown
in algorithm \ref{alg:Interactive,-inplace-timing}.
...
\hline
\hline 
SciPy & 622.3 ms & 2.62 ms\tabularnewline
\hline 
...
module for inlining C and C++ code. This can easily produce code 100x\cite{ramachandran-performance}
faster than pure Python. \cite{cai2005performance} showed that for
parrallel programming, mixed language solutions exhibit the same performance
gains from parralization as native language solutions.


\begin{itemize}
\item \cite{cai2005performance}
...
\item Pycam - All test and example code from this project, including framework
linking OpenCV to Pygame and NumPy. http://pycam.googlecode.com
\end{itemize}

\section{Conclusion}

We found that calling OpenCV from a Python wrapper was slower in general
than natively using the library. In getting these results we can't
help but note that the Python implementations were shorter, easier
to create and debug, and easier to test. Having and using an interactive
interpreter, to come to a solution prototype and being able to use
these prototypes in the final product is a big argument in favor of
Python. 
...
the additional tools and libraries available ..... 

Support for these packages, is almost entirely reliant on experianced
members of the community responding to message boards or mailing lists.
When one of the authors asked a question regarding the gaussian image
filtering function to the SciPy mailing list it spurred not only an
immediate response pointing out the solution to the direct query but
also a disscussion on finite approximations to gaussian kernals, how
OpenCV might do it differantly, and a user even went as far as to
test an IIR filter against the various implementations. This kind
of support is quite common in the open source world, where a package
...
Most of the time the reason for slow performance comes down to naive
implementations, which every language is prone to. 



A major limitations of using Python would be when the application
is being developed for special or embedded hardware. 

I would like to thank Rob Ramsey for help with profiling the execution
of C++ code to ensure accurate timing results were used. Thanks also
