...
application and also some standard operations used in CV such as image
acquisition. For each of the tests we describe the process, the difference
in terms of the syntax, the performance and the usability. All of
the testing code is made available open source at the project's website
\href{http://code.google.com/p/pycam/wiki/PythonComputerVision}{http://code.google.com/p/pycam/wiki/PythonComputerVision}.
...
against the SciPy library. Early results showed performance of OpenCV
called from Python performs similar to the native code making further
testing redundant. 
...
a most basic, but necessary test. This also serves to compare the
...
the framerate over a 2 minute period. Error bars are showing standard
...
for the Gaussian kernel\cite{ben1991image}.

The graph in Figure \ref{fig:Difference-in-Python} shows a 3 way
sub plot of a single image. This graph was created interactively using
the Python plotting package Matplotlib from IPython acting directly
...
While creating this program I found IPython particularly helpful,
...
which produced Figure \ref{fig:A-SciPy-harris}. The algorithm was
timed by executing it on Lena 300 times per sample:
...
\ref{fig:OpenCV-harris} for OpenCV. Note care has not been taken
to get the exact same output in OpenCV as in SciPy, it is assumed
that one could get the same output as shown earlier with the gaussian
blur example. Also note the algorithms are working on different dimensional
images, OpenCV is running on a 3 channel image whereas the SciPy version
is only working on a gray-scale image. Even so, OpenCV clearly runs
a magnitude faster than the unoptimized SciPy code. The main difference
between the algorithms is the threshold filtering and display of the
corner response. Another limitation is that the OpenCV response in
C++ is not implemented here.
...
We found that calling OpenCV from a Python wrapper was only slightly
slower in general than natively using the library. In getting these
results we note that the Python implementations were shorter, easier
...
The documentation was found to be complete, but not as extensive as
for a professional package like Matlab. Support for these open source
packages is almost entirely reliant on experienced members of the
community responding to message boards or mailing lists. When one
of the authors asked a question regarding the gaussian image filtering
function to the SciPy mailing list it spurred not only an immediate
response pointing out the solution to the direct query but also a
discussion on finite approximations to gaussian kernels, how OpenCV
might do it differently, and a user even went as far as to test an
IIR filter against the various implementations. This kind of support
is quite common in the open source world, where a package like SciPy
prides itself on its very rigorous and open tests. 
...
being developed for special or embedded hardware. 


\subsection*{Acknowledgments}

First and foremost I would like to thank Raphael Grasset for his mentoring
and guidance throughout this project. I would also like to thank Rob
Ramsey for help with profiling the execution of C++ code to ensure
accurate timing results were used. Thanks to all the tireless developers
investing time to work on open source projects OpenCV, Python and
SciPy.
