...
faster on average, but more varied. 
...
limitation is that the OpenCV response in C++ is not implemented here.
...
and \ref{fig:Angled-opencv-face}, this is caused because the classifier
was only trained on straight on face images. But in ideal conditions,
with good lighting and un-obscured faces directly facing the camera
the algorithm produces very robust results. There is no corresponding
high level functionality in SciPy, so a performance comparison was
not possible. A recent project PyCV\cite{tri2009principled} improves
on the face detection in OpenCV utilizing SciPy.
...
gaussian kernels, how OpenCV might do it differently.%
\footnote{A user even went as far as to test and propose a new IIR filter implementation
...
than pure Python. \cite{cai2005performance} showed that for parallel
programming, mixed language solutions exhibit the same performance
gains as native language solutions from moving to parallel solutions.
...
GPUCV projects both allow using this technology from Python\cite{lejdfors2006implementing}\cite{farrugia2006gpucv}\cite{allusse2008gpucv}.
...
PyPy, a compliant, flexible and fast implementation of the Python
Language. Google is currently midway through their \noun{Unladen Swallow}
project which aims to speed up Python by leveraging the Low Level
Virtual Machine (LLVM). All of these projects help make Python a safe
bet for the future of scientific computing and specifically for use
