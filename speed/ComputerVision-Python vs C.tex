...
In terms of time performance OpenCV (Python and C version) is twice faster than SciPy as visible Figure \ref{fig:performance-gaussian}.
%
...
faster on average, but the performance was more consistent in OpenCV C. 
...
are visible Figure \ref{fig:harris_scipy_static} on the Lena dataset for SciPy and OpenCV Python. A filter kernel size
...
Timing performances are available on the table below (measured on average on 300 iterations on the Lena dataset).
\begin{center}
\begin{tabular}{|c|c|c|}
\hline 
...
1 ms. There is no corresponding high level functionality for Face deteciton in SciPy,
so a performance comparison was not possible. 

However, A recent project PyCV \cite{tri2009principled} improves the face detection from OpenCV utilizing SciPy.
...
\caption{\label{fig:OpenCV-Face-Detection}Face Detection with OpenCV Python}
...
}\subfloat[\label{fig:edge-filtering-face}edge filtering and face detection]{\includegraphics[width=0.45\columnwidth]{report_data/pygame-face-edge}
...
In getting te development and results of the test we noticed that the Python implementations
